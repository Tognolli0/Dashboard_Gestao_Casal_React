    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-fade-up pb-10 text-slate-900">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-pink-50 pb-6">
                <div>
                    <h2 className="text-3xl font-black italic tracking-tighter">
                        Espaço <span className="text-pink-500">Dela</span>
                    </h2>
                    <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Beatriz — Pessoal & Secret Studio</p>
                </div>
                ...
                <div className="flex gap-3 flex-wrap">
                    <select
                        ...
                        className="bg-white border border-pink-100 rounded-xl px-4 py-2 text-sm font-bold text-pink-600 outline-none"
                    >
                        {MESES.map((m, i) => <option key={i} value={i}>{m}</option>)}
                    </select>

                    <div className="flex bg-pink-50/50 p-1 rounded-xl border border-pink-100">
                        {(['pessoal', 'studio'] as const).map(a => (
                            <button
                                key={a}
                                onClick={() => setAba(a)}
                                className={`px-4 py-2 rounded-lg text-xs font-black uppercase transition-all ${aba === a ? 'bg-white text-pink-600 shadow-sm' : 'text-pink-300'
                                    }`}
                            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard
                    label={aba === 'pessoal' ? 'Saldo Pessoal' : 'Saldo Studio'}
                    value={fmt(saldo)}
                    color={saldo >= 0 ? 'green' : 'pink'}
                />
            ...
            <Card className="p-6 border-pink-100">
                <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">
                    Novo Lançamento — {aba === 'pessoal' ? 'Pessoal' : 'Studio'}
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    <Input
                        ...
                        placeholder="Ex: Material, Receita..."
                    />
                    <Input
                        label="Valor (R$)"
                        type="number"
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {filtradas.map(t => (
                                <tr key={t.id} className="hover:bg-pink-50/20 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <Badge label={t.categoria} color="pink" />
                                    </td>
